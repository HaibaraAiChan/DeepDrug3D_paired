...
def site_voxelization(site, voxel_length):
    ...
    voxel = np.zeros( shape = (14, voxel_length, voxel_length, voxel_length),
        dtype = np.float64) 
    cnt = 0
    print "********voxel start ********"+str(voxel_start)
    print "---------voxel end -----------"+str(voxel_end)
   
    for x in xrange(voxel_start, voxel_end+1):
        for y in xrange(voxel_start, voxel_end+1):
            for z in xrange(voxel_start, voxel_end+1):
                temp_voxloc = [x,y,z]
...
class Vox3DBuilder(object):
    ...
    def voxelization(pdb_path, aux_input_path, r, N):
        ...
        out_pdb_dir = os.path.dirname(os.path.dirname(aux_input_path))+'/pdb_mol2/'
        out_pdb_name = os.path.basename(aux_input_path)[0:-4]
        output_trans_pdb_path =out_pdb_dir + out_pdb_name  + '_trans.pdb'
        print('Output the binding pocket aligned pdb file to: ' + output_trans_pdb_path)
               
       
 #       if os.path.exists(out_pdb_dir):
#		shutil.rmtree(out_pdb_dir)
 #       if not os.path.exists(out_pdb_dir):
#                os.makedirs(out_pdb_dir)

	
        if os.path.exists(output_trans_pdb_path):
		os.remove(output_trans_pdb_path)
        ...
        if os.path.exists(output_trans_mol2_path):
                os.remove(output_trans_mol2_path)
